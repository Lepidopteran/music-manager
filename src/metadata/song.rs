            ...
            "flac" => {
                ...
                Ok(Self {
                    ...
                    album_artist: comments
                        .album_artist()
                        .map(|album_artist| album_artist[0].to_string()),
                    genre: comments.genre().map(|genre| genre[0].to_string()),
                    track_number: comments.track().map(|track| track.to_string()),
                    disc_number: comments.get("DISCNUMBER").map(|disc| disc[0].to_string()),
                    ..Default::default()
                })
            }
            "mp4" | "m4a" => {
